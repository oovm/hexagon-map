use crate::{CubicPoint, HPoint, IsometricLine, Joint, Orientation, WPoint};
...
impl<T: Default> HexagonMap<T> {
    pub fn circle(diameter: usize) -> Self {
        ...
        for x in 0..diameter {
            for y in 0..diameter {
                let point = CubicPoint::new(x as isize, y as isize);
    ...
    pub fn rhombus(width: usize, height: usize) -> Self {
        ...
        for x in 0..width {
            for y in 0..height {
                map.insert(CubicPoint::new(x as isize, y as isize), Default::default());
            }
        }
        Self { sparse: map }
    }
